...
One way that we can permanently leave these questions behind us is if $m$ is an integer.  Notice that if $m=0$ or is an even integer, $u_1(x)$ terminates with a finite number of terms.\sidenote{i.e. If $m$ is even then $u_1(x)$ is a polynomial.} Similarly with $u_2(x)$ in the case that $m$ is an odd integer. 
