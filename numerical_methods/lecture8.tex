...
\marginnote[1.5cm]{
\ref{lst:ann8n-1} The built-in function \lstinline[style=myMatlab]{max()} takes a vector and returns the index of the largest element.  Of course the built-in function \lstinline[style=myMatlab]{abs()} returns a vector in which all of the elements are converted to their absolute value.

\vspace{0.25cm} 

\ref{lst:ann8n-2} The \lstinline[style=myMatlab]{max} function will return an index that needs to be corrected to reflect the actual row number in \lstinline[style=myMatlab]{A}.
